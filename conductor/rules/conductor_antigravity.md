...
-   **Artifact Gating Rule:** When using `ask_question` to approve `spec.md` or `plan.md`, the full text under review MUST be printed to the chat stream prior to rendering the modal. Blind approval prompts (e.g. asking for approval without printing the text) are strictly prohibited.
