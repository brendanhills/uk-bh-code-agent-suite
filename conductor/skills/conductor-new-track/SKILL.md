...
4.  **User Presentation & Confirmation (MANDATORY SEQUENCE):**
    -   **Pre-Prompt Output:** You MUST output the complete, drafted `plan.md` in your visible chat response using standard Markdown so the user has immediate visual access to the entire roadmap.
    -   **Interactive Gate:** ONLY after the complete plan is rendered in the chat stream may you trigger the `ask_question` approval modal asking the user to Approve or Revise.
    -   Never prompt for approval with only the track name; the plan content must always be visible above the prompt.
